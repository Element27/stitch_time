      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select
            ...
            className="w-full bg-[#242220] border border-[rgba(214,203,189,0.12)] rounded-lg px-2.5 py-1.5 text-xs text-[#FAF7F2] font-mono focus:border-[#C89B3C] focus:outline-none"
          >
            {logs.map((log) => (
        ...
        <div>
          ...
          <select
            ...
            className="w-full bg-[#242220] border border-[#C89B3C]/40 rounded-lg px-2.5 py-1.5 text-xs text-[#FAF7F2] font-mono focus:border-[#C89B3C] focus:outline-none"
          >
            {logs.map((log) => (
              <option key={log.id} value={log.id}>
                {formatDate(log.recorded_at)} ({log.notes ? log.notes.slice(0, 20) + '...' : log.unit})
      ...
      <div className="space-y-1.5 max-h-[300px] overflow-y-auto pr-1">
        {allFields.map((field) => {
          ...
          return (
            <div
              key={field}
              className="flex items-center justify-between p-2.5 rounded-xl bg-[#1E1D1B] hover:bg-[#242220] border border-[rgba(214,203,189,0.08)] transition-colors text-xs"
            >
              ...
              <div className="flex items-center gap-3 font-mono tnum">
                {/* Base Value */}
                <span className="text-[#9E948A]">
                  {formatMeasurement(val1, baseLog.unit)}
                </span>

                <span className="text-[10px] text-[#9E948A]/40">→</span>
                ...
                {delta !== null ? (
                  delta === 0 ? (
                    <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] bg-[#242220] text-[#9E948A]">
                      <Minus className="w-2.5 h-2.5" /> 0.0
                    </span>
                  ) : delta > 0 ? (
                    <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] bg-amber-950/60 text-amber-300 border border-amber-500/30">
                      <ArrowUpRight className="w-2.5 h-2.5" /> +{delta}&quot;
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] bg-emerald-950/60 text-emerald-300 border border-emerald-500/30">
                      <ArrowDownRight className="w-2.5 h-2.5" /> {delta}&quot;
                    </span>
                  )
                ) : (
                  <span className="text-[10px] text-[#9E948A]">—</span>
                )}
              </div>
            </div>
          );
